...
def print_results(results_dic, results_stats_dic, model,
    ...
    print("\n\n*** Results Summary for CNN Model Architecture", model.upper(),
          "***")
    print(" N Images: {n_images}".format(**results_stats_dic))
    print("N Dog Images: {n_dogs_img}".format(**results_stats_dic))
    print("N Not-Dog Images: {n_notdogs_img}".format(**results_stats_dic))
    ...
    for key in key_list:
        key_val = results_stats_dic[key]
        if re.match('pct', key) != None:
            print('\n' + key + ' : {}'.format(key_val))

    if results_stats_dic['n_correct_dogs'] != results_stats_dic['n_dogs_img']:
        diffrence = abs(
            results_stats_dic['n_correct_dogs'] - results_stats_dic['n_dogs_img'])
        print("\nINCORRECT Dog/NOT Dog Assignments: {}".format(diffrence))
    ...
    if results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed']:
        ...
        print("\nINCORRECT Dog Breed Assignment: {}".format(diffrence_1))

        if (results_dic[key][2] == 0 and (results_dic[key][3] + results_dic[key][4]) == 2):
